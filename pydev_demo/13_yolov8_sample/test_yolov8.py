...
################################################################################
# YOLOv8 BPU Inference Sample - Based on YOLOv5 Sample Structure
################################################################################
...
if __name__ == '__main__':
    ...
    t1 = time.time()
    print("inference time is :", (t1 - t0))

    # 获取结构体信息
    yolov8_postprocess_info = Yolov5PostProcessInfo_t()
    yolov8_postprocess_info.height = h
    yolov8_postprocess_info.width = w
    org_height, org_width = img_file.shape[0:2]
    yolov8_postprocess_info.ori_height = org_height
    yolov8_postprocess_info.ori_width = org_width
    yolov8_postprocess_info.score_threshold = 0.4 
    yolov8_postprocess_info.nms_threshold = 0.45
    yolov8_postprocess_info.nms_top_k = 20
    yolov8_postprocess_info.is_pad_resize = 0
    ...
    for i in range(len(models[0].outputs)):
        ...
        libpostprocess.Yolov5doProcess(output_tensors[i], ctypes.pointer(yolov8_postprocess_info), i)

    result_str = get_Postprocess_result(ctypes.pointer(yolov8_postprocess_info))  
    ...
    cv2.imwrite('yolov8_output_image.jpg', img_file)
    print("YOLOv8 inference completed! Output saved to yolov8_output_image.jpg")