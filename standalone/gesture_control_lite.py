#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""gesture_control_lite.py - 轻量版手势识别 (独立文件, 与完整版 gesture_control.py 无关)

与完整版的区别 (省 CPU):
  - 默认不启动 MJPEG 推流, 跳过画面绘制和 JPEG 编码 (调试可加 --stream 开启)
  - 默认限制处理帧率 10fps (--proc-fps), MediaPipe 全速跑会占满一核
...
运行:
  python3 gesture_control_lite.py --device /dev/video0 --udp-port 5005
  python3 gesture_control_lite.py --device /dev/video0 --stream   # 临时开推流调试 (8094)
"""
import os
import sys
import time
...
class ActionScheduler:
    ...
    def _send_stop(self, now: float):
        ...
        self.udp.send_action('stop')
        if now - self.last_log_time > self.log_interval:
            print(f"[gesture] 无手势, 停车 (was {action})")
            self.last_log_time = now
...
class LatestFrameReader:
    """后台线程持续读摄像头, 主循环永远拿最新一帧 (消除 V4L2 缓冲区积压延迟)"""
...
def main():
    ...
    parser.add_argument('--port', type=int, default=8094, help='HTTP MJPEG 推流端口 (仅 --stream 时生效)')
    parser.add_argument('--stream', action='store_true',
                        help='开启 MJPEG 推流调试 (默认关闭, 省 CPU)')
    parser.add_argument('--proc-fps', type=float, default=10,
                        help='限制手势识别处理帧率 (默认 10, 0=不限制; MediaPipe 全速跑约占满一核)')
    ...
    # 4. MJPEG 推流: 轻量版默认关闭, --stream 调试时才启动 (后台线程)
    ...
    if args.stream:
        ...
        print("[server] 轻量模式: MJPEG 推流关闭 (调试加 --stream)")
    ...
    fps_cnt = 0
    last_seq = 0
    min_interval = 1.0 / args.proc_fps if args.proc_fps > 0 else 0.0
    if min_interval > 0:
        print(f"[main] 处理帧率限制: {args.proc_fps} fps (省 CPU)")
    try:
        while True:
            loop_t0 = time.time()
            ...
            # 限帧节流: 补足本帧剩余时间 (MediaPipe 是 CPU 大户)
            if min_interval > 0:
                remain = min_interval - (time.time() - loop_t0)
                if remain > 0:
                    time.sleep(remain)
