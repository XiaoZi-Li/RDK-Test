...
# -*- coding: utf-8 -*-
"""
test_yolov5s_v6.py - YOLOv5s V6版本目标检测推理示例
================================================================================

【程序功能】
本程序使用YOLOv5s V6版本（Ultralytics YOLOv5的第六版迭代）目标检测模型进行图像推理。
YOLOv5s是YOLOv5系列中的"small"版本，在速度和精度之间取得良好平衡。
...
【YOLOv5 V6版本改进】
相比之前的V5版本，V6版本的改进包括：
1. 骨干网络优化：使用更高效的CSP结构
2. NMS优化：改进非极大值抑制的效率
3. 推理速度提升：针对边缘设备优化
4. 量化支持增强：更好的INT8量化精度

【YOLOv5系列对比】
| 模型      | 输入尺寸 | 参数量  | mAP@0.5 | 推理速度（边缘） |
|-----------|----------|---------|---------|-----------------|
| YOLOv5s  | 640x640  | 7.2M    | 56.0%   | 非常快          |
| YOLOv5m  | 640x640  | 21.2M   | 63.1%   | 快              |
| YOLOv5l  | 640x640  | 46.5M   | 67.1%   | 中等            |
| YOLOv5x  | 672x672  | 86.7M   | 68.7%   | 较慢            |
...
# 【YOLOv5s V6 vs V7 对比】
# ================================================================================
# YOLOv5 V6和V7都是Ultralytics的重要迭代版本
# | 特性         | V6                | V7                |
# |--------------|-------------------|-------------------|
# | 发布时间     | 2021年10月        | 2022年7月        |
# | 主干网络     | CSPDarknet        | CSPDarknet + TFF |
# | 颈部网络     | SPPF + PAN        | SPPF + PAN       |
# | 训练策略     | 标准增强          | 更强数据增强      |
# | 精度提升     | 基准              | +1-2% mAP        |
# | 速度变化     | 基准              | 略慢（但精度更高）|
#
# V7版本引入了更强的数据增强和新的网络模块（如TFF Transformer）
