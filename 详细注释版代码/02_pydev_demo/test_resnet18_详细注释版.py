...
# -*- coding: utf-8 -*-
"""
test_resnet18.py - ResNet18模型图像分类推理示例
================================================================================

【程序功能】
本程序使用ResNet18（Residual Network，残差网络）模型对图像进行分类推理。
ResNet18是2015年ImageNet竞赛的冠军模型ResNet系列中最轻量的版本。
...
【ResNet核心创新：残差连接（Residual Connection）】
1. 引入"快捷连接"（Shortcut Connection）绕过卷积层
2. 解决深层网络梯度消失问题
3. 使训练更深层次的网络成为可能
4. 让信息直接流动，减少信息损失

【ResNet18 vs 其他模型】
- ResNet18：18层，~11.7M参数，适合轻量级应用
- ResNet50：50层，~25.5M参数，更高精度
- GoogleNet：22层，~5M参数，Inception结构
...
class hbDNNQuantiScale_t(ctypes.Structure):
    """
    hbDNNQuantiScale_t - 量化比例参数
    用于非对称量化中的scale和zero_point
    """
    _fields_ = [
...
class ClassificationPostProcessInfo_t(ctypes.Structure):
    ...
    【与EfficientNASNet/GoogleNet的区别】
        本程序设置了更大的nms_top_k=500，意味着会返回更多候选结果
        这对于需要多个候选结果的应用场景很有用
    """
    _fields_ = [
        ("height",ctypes.c_int),
...
if __name__ == '__main__':
    ...
    # 【注意】这里设置nms_top_k=500，返回更多候选结果
    # 相比之下，GoogleNet/EfficientNASNet只返回1个结果
...
# 【ResNet系列一览】
# ================================================================================
# | 模型      | 层数 | 参数量   | ImageNet Top-1 | 特点                    |
# |-----------|------|----------|---------------|-------------------------|
# | ResNet18  | 18   | 11.7M    | 69.8%         | 轻量，适合嵌入式         |
# | ResNet34  | 34   | 21.8M    | 73.3%         | 平衡精度与速度           |
# | ResNet50  | 50   | 25.5M    | 76.1%         | 高精度                   |
# | ResNet101 | 101  | 44.5M    | 77.4%         | 更高精度                 |
#
# 【残差块结构】
# 输入 -> Conv3x3 -> BN -> ReLU -> Conv3x3 -> BN -> Add -> ReLU -> 输出
#           |                                           ^
#           +----------- Shortcut Connection ------------+
