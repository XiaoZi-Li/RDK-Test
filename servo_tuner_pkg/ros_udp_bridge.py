...
class RosUdpBridge(Node):
    def __init__(self):
        super().__init__('ros_udp_bridge_servo_tuner')
        ...
        self.get_logger().info(
            f'[servo_tuner] ros_udp_bridge started. '
    ...
    def action_callback(self, msg: String):
        ...
        try:
            ...
            if isinstance(payload, dict):
                ...
                if mode == 'follow_control' or mode == 'servo_control':
                    # 连续控制 / 舵机精细控制 直接透传 JSON
                    self.sock.sendto(
                        raw.encode('utf-8'),
                    ...
                    if mode == 'servo_control':
                        self.get_logger().info(
                            f'UDP servo_control: count={len(payload.get("servos", []))}'
                        )
                    return
