#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ws_bridge_node.py - 运动调节专用 WebSocket 桥接节点

本文件为 servo_tuner_pkg 独立运行版本，不修改原 puppy_brain 包。
与原版本相比，额外支持 {type:'servo_control'} 消息。

启动前需先 source ROS2 环境：
...
class WsBridgeNode(Node):
    def __init__(self):
        super().__init__('ws_bridge_node_servo_tuner')
        ...
        self.get_logger().info(f'[servo_tuner] ws_bridge_node started. WS port={WS_PORT}')
    ...
    async def _handle_command(self, raw: str):
        ...
        elif t == 'servo_control':
            # 运动调节上位机：单舵机/多舵机精细控制
            servos = obj.get('servos', [])
            if not isinstance(servos, list) or not servos:
                return
            payload = {
                'mode': 'servo_control',
                'servos': servos,
                'source': obj.get('source', 'pc_servo_tuner'),
                'timestamp': now,
            }
            msg = String()
            msg.data = json.dumps(payload, ensure_ascii=False)
            self.action_pub.publish(msg)
            self.get_logger().info(f'[PC->servo] count={len(servos)}')
