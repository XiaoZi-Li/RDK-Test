...
# =========================================================
# 路径适配：当本文件不在原 /app/pydev_demo/puppypi_control 目录时，
# 自动把原目录加入 sys.path，以便导入 HiwonderPuppy / servo_controller 等
# =========================================================
_HERE = os.path.dirname(os.path.abspath(__file__))
_CANDIDATES = [
    '/app/pydev_demo/puppypi_control',
    os.path.join(os.path.dirname(_HERE), 'pydev_demo', 'puppypi_control'),
]
...
def do_servo_control(servos):
    """
    运动调节上位机：直接控制单个或多个舵机
    servos: [{id:1, pulse:1500, time:200}, ...]
    """
    global robot_pose_state
    ...
    for item in servos:
        try:
            ...
            pulse = int(item.get('pulse', 1500))
            use_time = int(item.get('time', 200))
            if 1 <= sid <= 11:
                setServoPulse(sid, pulse, use_time)
        except Exception as e:
            print(f"⚠️ servo_control 单条解析失败: {item}, {e}")

    # 直接调舵机后，机体姿态不再是已知步态状态
    robot_pose_state = "unknown"
    print(f"✅ servo_control: 已发送 {len(servos)} 个舵机目标")
...
try:
    while True:
        ...
        if action == "servo_control":
            # 运动调节上位机：直接控制舵机，每帧都执行，不做去重
            try:
                servos = payload.get("servos", [])
                if servos:
                    do_servo_control(servos)
            except Exception as e:
                print(f"⚠️ servo_control 执行异常: {e}")
            continue
